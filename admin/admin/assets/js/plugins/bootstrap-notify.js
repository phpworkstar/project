...
(function(factory) {
  if (typeof define === 'function' && define.amd) {
    // AMD. Зарегистрировать, как анонимный модуль.
 ...
 // Создаем значения по умолчанию один раз
  var defaults = {
    element: 'body',
    position: null,
    type: "info",
...
// Входная строка может отличаться от фактической проанализированной строки HTML!
       // (например, <br> против <br />)
       // Итак, здесь мы должны принудительно разобрать это как HTML!
  ...
  function Notify(element, content, options) {
    ...
    //если повторяющиеся сообщения не допускаются, продолжайте только в том случае, если это новое сообщение не является дубликатом того, которое уже отображается
